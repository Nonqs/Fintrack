"use client"
import { Alert, IconButton, Typography } from "@mui/joy"
import CloseRoundedIcon from '@mui/icons-material/CloseRounded';
import WarningIcon from '@mui/icons-material/Warning';
import { Button, Card, CardBody, CardHeader, Input } from "@nextui-org/react"
import { useEffect, useState } from "react"

export default function ShowAccounts() {

    const [accounts, setAccounts] = useState([])
    const [alert, setAlert] = useState(false)
    const [warning, setWarning] = useState(false)

    useEffect(() => {

        const getAccounts = async () => {

            const res = await fetch("/api/accounts", {
                method: "GET",
                headers: {
                    "Content-Type": "application/json"
                }
            })
            const data = await res.json()
            setAccounts(data)
        }

        getAccounts()
    }, [])

    const [editName, setEditName] = useState<string>("")
    const [editIndex, setEditIndex] = useState(-1)

    const handleEdit = (index: number) => {

        setEditIndex(index)
    }

    const handleSave = async (index: number) => {

        const actualName = accounts[index].name

        const res = await fetch("/api/accounts", {
            method: "PATCH",
            body: JSON.stringify({
                actualName,
                editName
            }),
            headers: {
                "Content-Type": "application/json"
            }
        })

        accounts[index].name = editName
        setEditIndex(-1)
    }

    const handleDelete = async (index: number) => {
        const name = accounts[index].name;

        if (!alert) {
            setAlert(true);
            setWarning(true);
        } else if (warning) {
            setAlert(false);
            setWarning(false);

            const res = await fetch("/api/accounts", {
                method: "DELETE",
                body: JSON.stringify({ name }),
                headers: {
                    "Content-Type": "application/json"
                }
            })

            if (res.ok) {

                accounts.splice(index, 1);
                setAccounts([...accounts])
                setWarning(false);
            } else {

                console.error('Error deleting account');
            }

        }
    }

    return (
        <Card className="w-full h-full">
            <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
                <h4 className="font-bold text-large border-b-1">Accounts</h4>
            </CardHeader>
            <CardBody>
                {accounts.map((account, index) => (
                    <div className="bg-default-100 rounded mb-2 p-1">
                        <div className="flex justify-between">
                            <article className="flex items-center">
                                {editIndex === index
                                    ? <Input className="border rounded-xl" value={editName} onChange={(e) => { setEditName(e.target.value) }} />
                                    : <span className="first-letter:uppercase">{account.name}</span>
                                }
                            </article>
                            <article>
                                {editIndex === index
                                    ? (
                                        <article className="flex">
                                            <Button onClick={() => handleSave(index)} size="sm" className="bg-green-400 me-2" isIconOnly color="warning" variant="faded" aria-label="Cancel">
                                                <svg className="w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fillRule="evenodd" clip-rule="evenodd" d="M18.1716 1C18.702 1 19.2107 1.21071 19.5858 1.58579L22.4142 4.41421C22.7893 4.78929 23 5.29799 23 5.82843V20C23 21.6569 21.6569 23 20 23H4C2.34315 23 1 21.6569 1 20V4C1 2.34315 2.34315 1 4 1H18.1716ZM4 3C3.44772 3 3 3.44772 3 4V20C3 20.5523 3.44772 21 4 21L5 21L5 15C5 13.3431 6.34315 12 8 12L16 12C17.6569 12 19 13.3431 19 15V21H20C20.5523 21 21 20.5523 21 20V6.82843C21 6.29799 20.7893 5.78929 20.4142 5.41421L18.5858 3.58579C18.2107 3.21071 17.702 3 17.1716 3H17V5C17 6.65685 15.6569 8 14 8H10C8.34315 8 7 6.65685 7 5V3H4ZM17 21V15C17 14.4477 16.5523 14 16 14L8 14C7.44772 14 7 14.4477 7 15L7 21L17 21ZM9 3H15V5C15 5.55228 14.5523 6 14 6H10C9.44772 6 9 5.55228 9 5V3Z" fill="#ffffff"></path> </g></svg>
                                            </Button>
                                            <Button onClick={() => setEditIndex(-1)} size="sm" className="bg-red-400 me-2" isIconOnly color="warning" variant="faded" aria-label="Cancel">
                                                <svg fill="#ffffff" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>cancel1</title> <path d="M16 29c-7.18 0-13-5.82-13-13s5.82-13 13-13 13 5.82 13 13-5.82 13-13 13zM16 6c-5.522 0-10 4.478-10 10s4.478 10 10 10c5.523 0 10-4.478 10-10s-4.477-10-10-10zM20.537 19.535l-1.014 1.014c-0.186 0.186-0.488 0.186-0.675 0l-2.87-2.87-2.87 2.87c-0.187 0.186-0.488 0.186-0.675 0l-1.014-1.014c-0.186-0.186-0.186-0.488 0-0.675l2.871-2.869-2.871-2.87c-0.186-0.187-0.186-0.489 0-0.676l1.014-1.013c0.187-0.187 0.488-0.187 0.675 0l2.87 2.87 2.87-2.87c0.187-0.187 0.489-0.187 0.675 0l1.014 1.013c0.186 0.187 0.186 0.489 0 0.676l-2.871 2.87 2.871 2.869c0.186 0.187 0.186 0.49 0 0.675z"></path> </g></svg>
                                            </Button>
                                        </article>

                                    )
                                    : (
                                        <article className="flex">
                                            <Button onClick={() => { handleEdit(index), setEditName(account.name) }} size="sm" className="bg-yellow-400 me-2" isIconOnly color="warning" variant="faded" aria-label="Edit">
                                                <svg viewBox="0 -0.5 25 25" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M13.2942 7.95881C13.5533 7.63559 13.5013 7.16358 13.178 6.90453C12.8548 6.64549 12.3828 6.6975 12.1238 7.02072L13.2942 7.95881ZM6.811 14.8488L7.37903 15.3385C7.38489 15.3317 7.39062 15.3248 7.39623 15.3178L6.811 14.8488ZM6.64 15.2668L5.89146 15.2179L5.8908 15.2321L6.64 15.2668ZM6.5 18.2898L5.7508 18.2551C5.74908 18.2923 5.75013 18.3296 5.75396 18.3667L6.5 18.2898ZM7.287 18.9768L7.31152 19.7264C7.36154 19.7247 7.41126 19.7181 7.45996 19.7065L7.287 18.9768ZM10.287 18.2658L10.46 18.9956L10.4716 18.9927L10.287 18.2658ZM10.672 18.0218L11.2506 18.4991L11.2571 18.491L10.672 18.0218ZM17.2971 10.959C17.5562 10.6358 17.5043 10.1638 17.1812 9.90466C16.8581 9.64552 16.386 9.69742 16.1269 10.0206L17.2971 10.959ZM12.1269 7.02052C11.8678 7.34365 11.9196 7.81568 12.2428 8.07484C12.5659 8.33399 13.0379 8.28213 13.2971 7.95901L12.1269 7.02052ZM14.3 5.50976L14.8851 5.97901C14.8949 5.96672 14.9044 5.95412 14.9135 5.94123L14.3 5.50976ZM15.929 5.18976L16.4088 4.61332C16.3849 4.59344 16.3598 4.57507 16.3337 4.5583L15.929 5.18976ZM18.166 7.05176L18.6968 6.52192C18.6805 6.50561 18.6635 6.49007 18.6458 6.47532L18.166 7.05176ZM18.5029 7.87264L19.2529 7.87676V7.87676L18.5029 7.87264ZM18.157 8.68976L17.632 8.15412C17.6108 8.17496 17.5908 8.19704 17.5721 8.22025L18.157 8.68976ZM16.1271 10.0203C15.8678 10.3433 15.9195 10.8153 16.2425 11.0746C16.5655 11.3339 17.0376 11.2823 17.2969 10.9593L16.1271 10.0203ZM13.4537 7.37862C13.3923 6.96898 13.0105 6.68666 12.6009 6.74805C12.1912 6.80943 11.9089 7.19127 11.9703 7.60091L13.4537 7.37862ZM16.813 11.2329C17.2234 11.1772 17.5109 10.7992 17.4552 10.3888C17.3994 9.97834 17.0215 9.69082 16.611 9.74659L16.813 11.2329ZM12.1238 7.02072L6.22577 14.3797L7.39623 15.3178L13.2942 7.95881L12.1238 7.02072ZM6.24297 14.359C6.03561 14.5995 5.91226 14.9011 5.89159 15.218L7.38841 15.3156C7.38786 15.324 7.38457 15.3321 7.37903 15.3385L6.24297 14.359ZM5.8908 15.2321L5.7508 18.2551L7.2492 18.3245L7.3892 15.3015L5.8908 15.2321ZM5.75396 18.3667C5.83563 19.1586 6.51588 19.7524 7.31152 19.7264L7.26248 18.2272C7.25928 18.2273 7.25771 18.2268 7.25669 18.2264C7.25526 18.2259 7.25337 18.2249 7.25144 18.2232C7.2495 18.2215 7.24825 18.2198 7.24754 18.2185C7.24703 18.2175 7.24637 18.216 7.24604 18.2128L5.75396 18.3667ZM7.45996 19.7065L10.46 18.9955L10.114 17.536L7.11404 18.247L7.45996 19.7065ZM10.4716 18.9927C10.7771 18.9151 11.05 18.7422 11.2506 18.499L10.0934 17.5445C10.0958 17.5417 10.0989 17.5397 10.1024 17.5388L10.4716 18.9927ZM11.2571 18.491L17.2971 10.959L16.1269 10.0206L10.0869 17.5526L11.2571 18.491ZM13.2971 7.95901L14.8851 5.97901L13.7149 5.04052L12.1269 7.02052L13.2971 7.95901ZM14.9135 5.94123C15.0521 5.74411 15.3214 5.6912 15.5243 5.82123L16.3337 4.5583C15.4544 3.99484 14.2873 4.2241 13.6865 5.0783L14.9135 5.94123ZM15.4492 5.7662L17.6862 7.6282L18.6458 6.47532L16.4088 4.61332L15.4492 5.7662ZM17.6352 7.58161C17.7111 7.6577 17.7535 7.761 17.7529 7.86852L19.2529 7.87676C19.2557 7.36905 19.0555 6.88127 18.6968 6.52192L17.6352 7.58161ZM17.7529 7.86852C17.7524 7.97604 17.7088 8.07886 17.632 8.15412L18.682 9.22541C19.0446 8.87002 19.2501 8.38447 19.2529 7.87676L17.7529 7.86852ZM17.5721 8.22025L16.1271 10.0203L17.2969 10.9593L18.7419 9.15928L17.5721 8.22025ZM11.9703 7.60091C12.3196 9.93221 14.4771 11.5503 16.813 11.2329L16.611 9.74659C15.0881 9.95352 13.6815 8.89855 13.4537 7.37862L11.9703 7.60091Z" fill="#ffffff"></path> </g></svg>
                                            </Button>
                                            <Button onClick={() => { handleDelete(index) }} size="sm" className="bg-red-400" isIconOnly color="danger" variant="faded" aria-label="Take a photo">
                                                <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M18 6L17.1991 18.0129C17.129 19.065 17.0939 19.5911 16.8667 19.99C16.6666 20.3412 16.3648 20.6235 16.0011 20.7998C15.588 21 15.0607 21 14.0062 21H9.99377C8.93927 21 8.41202 21 7.99889 20.7998C7.63517 20.6235 7.33339 20.3412 7.13332 19.99C6.90607 19.5911 6.871 19.065 6.80086 18.0129L6 6M4 6H20M16 6L15.7294 5.18807C15.4671 4.40125 15.3359 4.00784 15.0927 3.71698C14.8779 3.46013 14.6021 3.26132 14.2905 3.13878C13.9376 3 13.523 3 12.6936 3H11.3064C10.477 3 10.0624 3 9.70951 3.13878C9.39792 3.26132 9.12208 3.46013 8.90729 3.71698C8.66405 4.00784 8.53292 4.40125 8.27064 5.18807L8 6" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                                            </Button>
                                        </article>
                                    )
                                }
                            </article>
                        </div>
                        <div className="w-1/2 flex justify-between">
                            <small className="text-default-400 font-semibold">Balance: </small>
                            <span>${account.cash}</span>
                        </div>
                        {alert &&
                            <div className="fixed bottom-1 right-1 z-50">
                                <Alert
                                    sx={{ alignItems: 'flex-start' }}
                                    startDecorator={<WarningIcon />}
                                    variant="soft"
                                    color={"warning"}
                                    endDecorator={
                                        <IconButton onClick={() => { setAlert(false); setWarning(false); }} variant="soft" color={"warning"}>
                                            <CloseRoundedIcon />
                                        </IconButton>
                                    }
                                >
                                    <div>
                                        <div>Warning</div>
                                        <Typography level="body-sm" color={"warning"}>
                                            <span>Are you sure you want to delete this account?<br /> This action is irreversible and deletes all the info that is connected with the account.</span>
                                            <br />
                                            <Button
                                                onClick={() => {
                                                    setWarning(true);
                                                    handleDelete(index);
                                                }}
                                                className="me-5 mt-2"
                                                color="danger"
                                            >
                                                Delete
                                            </Button>
                                            <Button
                                                onClick={() => {
                                                    setAlert(false);
                                                    setWarning(false);
                                                }}
                                                className="mt-2"
                                                color="warning"
                                            >
                                                Cancel
                                            </Button>
                                        </Typography>
                                    </div>
                                </Alert>
                            </div>
                        }
                    </div>
                ))}
            </CardBody>
        </Card>
    )
}