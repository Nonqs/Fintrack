...
export default function ShowIncomes() {
    ...
    const handleSave = async (index: number) => {
        ...
        const actualName = incomesType[index].name
        ...
        incomesType[index].name = editName
        setEditIndex(-1)
    ...
    const handleDelete = async (index: number) => {
        ...
        const name = incomesType[index].name
        ...
        if (!alert) {
            ...
            if (res.ok) {
                ...
                incomesType.splice(index, 1)
                setIncomesType([...incomesType])
                setWarning(false)
            } else {
    ...
    return (
        <Card className="w-full h-full">
            <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
                <h4 className="font-bold text-large">Income Types</h4>
            </CardHeader>
            <CardBody>
                {incomesType.map((income, index) => (
                    <div className="bg-default-100 rounded mb-2 p-2">
                        <div className="flex justify-between">
                            ...
                            <article>
                                {editIndex === index
                                    ...
                                    : (
                                        <article className="flex">
                                            <Button onClick={() => { handleEdit(index), setEditName(income.name) }} size="sm" className="bg-yellow-400 me-2" isIconOnly color="warning" variant="faded" aria-label="Edit">
